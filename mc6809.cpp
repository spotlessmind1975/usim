...
void mc6809::reset(Word _pc, Word _trap)
{
	USim::reset();
...
void mc6809::tick()
{
	...
	if (trace)
	{
		printf("[%4.4x] %2.2x A: %2.2x B: %2.2x X: %4.4x Y: %4.4x S: %4.4x [%2.2x] CC: %2.2X\n\r", pc, read(pc), a, b, x, y, s, read(s), cc.all);
	}
...
void mc6809::pre_exec()
{
	...
	// int base = 0x2C1F;
	// fprintf(stderr, "DIVIDED:%02X%02X%02X%02X QUOTIENT:%02X%02X%02X%02X CQUOTIENT:%02X%02X%02X%02X MODULUS:%02X%02X%02X%02X\r\n",
	// 	read(base), read(base+1), read(base+2), read(base+3),
	// 	read(base+4), read(base+5), read(base+6), read(base+7),
	// 	read(base+8), read(base+9), read(base+10), read(base+11),
	// 	read(base+12), read(base+13), read(base+14), read(base+15));

	int i = 0;
	fprintf(stderr, "RESBUFFER:");
	int base = 0x2AD4;
	for (i = 0; i < 10; ++i)
	{
		Byte v = read(base + i);
		fprintf(stderr, "%2.2x", (unsigned char)v);
	}
	fprintf(stderr, " - ");
	for (i = 0; i < 10; ++i)
	{
		...
		if (v >= '0' && v <= '9')
		{
			fprintf(stderr, "%c", (unsigned char)v);
		}
		else
		{
			fprintf(stderr, "?");
...
void mc6809::post_exec()
{
	...
	char *listing_instruction = NULL;
	if (m_listing)
	{
		listing_instruction = m_listing[insn_pc];
	}

	fprintf(stderr, "/ %04X: [%2d] %-8s%s %s\r\n", insn_pc, cycles, insn, disasm_operand().c_str(), listing_instruction ? listing_instruction : "(none)");
}
