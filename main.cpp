...
static int doListing(int argc, char **argv)	/* -i file */
{
  ...
  while(!feof(handle) ) {
	...
	if ( strlen( instructions ) == 0 ) continue;
...
int main(int argc, char *argv[])
{
	...
	while (++argv, --argc > 0) {
		...
		else if (!strcmp(*argv, "-l"))	{ loading_address = htol(*(argv+1)); n = 1; }
		else if (!strcmp(*argv, "-R"))	{ pc = htol(*(argv+1)); n = 1; }
		else if (!strcmp(*argv, "-L"))	n= doLabels(argc, argv);
		else if (!strcmp(*argv, "-L2"))	n= doLabels2(argc, argv);
		else if (!strcmp(*argv, "-Li"))	n= doInspections(argc, argv);
		else if (!strcmp(*argv, "-O"))	{ output_filename = strdup( *(argv+1) ); n = 1; }
		else if (!strcmp(*argv, "-X"))	{ trap = htol(*(argv+1)); n = 1; }
		else if (!strcmp(*argv, "-t"))	{ trace = true; }
		else if (!strcmp(*argv, "-i"))	n= doListing(argc, argv);
	...
	if ( output_filename ) {
		...
		if ( 
				strcmp( label_name[i], "WORKING") == 0 
				|| strcmp( label_name[i], "TEMPORARY") == 0 
				|| strcmp( label_name[i], "DESCRIPTORS") == 0 
				) {
						fprintf(handle, "%s\n", label_name[i]);
						if ( strcmp( label_name[i], "WORKING") == 0 
								|| strcmp( label_name[i], "TEMPORARY") == 0 ) {
						max = 1024;
